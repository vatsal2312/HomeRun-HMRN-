...
constructor(string memory _name, string memory _symbol, address _token) ERC20(_name,
_symbol) {
...
function addLiquidity( address tokenA, address tokenB, uint amountADesired, uint
amountBDesired, uint amountAMin, uint amountBMin, address to, uint deadline ) external
returns (uint amountA, uint amountB, uint liquidity);
function addLiquidityETH( address token, uint amountTokenDesired, uint amountTokenMin,
uint amountETHMin, address to, uint deadline ) external payable returns (uint
amountToken, uint amountETH, uint liquidity);
function removeLiquidity( address tokenA, address tokenB, uint liquidity, uint
amountAMin, uint amountBMin, address to, uint deadline ) external returns (uint
amountA, uint amountB);
function removeLiquidityETH( address token, uint liquidity, uint amountTokenMin, uint
amountETHMin, address to, uint deadline ) external returns (uint amountToken, uint
amountETH);
function removeLiquidityWithPermit( address tokenA, address tokenB, uint liquidity,
uint amountAMin, uint amountBMin, address to, uint deadline, bool approveMax, uint8 v,
bytes32 r, bytes32 s ) external returns (uint amountA, uint amountB);
function removeLiquidityETHWithPermit( address token, uint liquidity, uint
amountTokenMin, uint amountETHMin, address to, uint deadline, bool approveMax, uint8
v, bytes32 r, bytes32 s ) external returns (uint amountToken, uint amountETH);
...
function swapTokensForExactTokens( uint amountOut, uint amountInMax, address[]
calldata path, address to, uint deadline ) external returns (uint[] memory amounts);
...
path, address to, uint deadline) external returns (uint[] memory amounts);
...
function removeLiquidityETHSupportingFeeOnTransferTokens( address token, uint
liquidity, uint amountTokenMin, uint amountETHMin, address to, uint deadline )
external returns (uint amountETH);
function removeLiquidityETHWithPermitSupportingFeeOnTransferTokens( address token,
uint liquidity, uint amountTokenMin, uint amountETHMin, address to, uint deadline,
bool approveMax, uint8 v, bytes32 r, bytes32 s ) external returns (uint amountETH);
function swapExactTokensForTokensSupportingFeeOnTransferTokens( uint amountIn, uint
...
function swapExactTokensForETHSupportingFeeOnTransferTokens( uint amountIn, uint
amountOutMin, address[] calldata path, address to, uint deadline ) external;
}
////////////////////////////////
////////// Libraries ///////////
...
DividendPayingToken("Boda_Busd_Dividend_Tracker",
"Boda_Busd_Dividend_Tracker",_dividentToken) {
claimWait = 3600;
...
"Boda_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
...
uint256 public sellFeeIncreaseFactor = 10;
...
event MarketingWalletUpdated(address indexed newMarketingWallet, address indexed
oldMarketingWallet);
...
constructor() ERC20("HomeRun", "HMRN") {
marketingWallet = 0xd11AC6068B38C850Dc2318845b0d8EF7Eb8AEC01;
...
require(_newWallet != marketingWallet, "HMRN: The marketing wallet is already this address");
...
require(newAddress != address(busdDividendTracker), "HMRN: The dividend tracker already has that address");
BUSDDividendTracker newbusdDividendTracker =
BUSDDividendTracker(payable(newAddress));
require(newbusdDividendTracker.owner() == address(this), "HMRN: The new dividend tracker must be owned by the Boda token contract");
...
require(newAddress != address(uniswapV2Router), "HMRN: The router already has that address");
emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
uniswapV2Router = IUniswapV2Router02(newAddress);
}
function excludeFromFees(address account, bool excluded) public onlyOwner {
require(isExcludedFromFees[account] != excluded, "HMRN: Account is already exluded from fees");
...
require(pair != uniswapV2Pair, "HMRN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
_setAutomatedMarketMakerPair(pair, value);
}
function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
require(automatedMarketMakerPairs[pair] != value, "HMRN: Automated market maker pair is already set to that value");
...
require(newValue != gasForProcessing, "HMRN: Cannot update gasForProcessing to same value");
...
function busdDividendTokenBalanceOf(address account) external view returns
(uint256) {
...
(uint256 busdIterations, uint256 busdClaims, uint256 busdLastProcessedIndex) =
busdDividendTracker.process(gas);
emit ProcessedbusdDividendTracker(busdIterations, busdClaims,
busdLastProcessedIndex, false, gas, tx.origin);
}
...
require(tradingIsEnabled || (isExcludedFromFees[from] ||
isExcludedFromFees[to]), "HMRN: Trading has not started yet");
...
amount.mul(busdDividendRewardsFee).div(100);
...
busdDividedRewardsInContract =
busdDividedRewardsInContract.add(tmpBusdDividedRewardsInContract);
...
try busdDividendTracker.process(gas) returns (uint256 iterations, uint256
claims, uint256 lastProcessedIndex) {
emit ProcessedbusdDividendTracker(iterations, claims,
...
function swapTokensForDividendToken(uint256 _tokenAmount, address _recipient,
address _dividendAddress) private {
...
transferDividends(busdDividendToken, address(busdDividendTracker),
busdDividendTracker, busdDividends);
}
function transferToWallet(address payable recipient, uint256 amount) private {
recipient.transfer(amount);
}
function transferDividends(address dividendToken, address dividendTracker,
DividendPayingToken dividendPayingTracker, uint256 amount) private {
